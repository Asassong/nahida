...
def deal_energy(energy_dict):
    ...
    for avatar in avatar_obj_dict.values():
        for max_energy_id in range(70, 77):
            element_energy = fight_prop_dict[max_energy_id]
...
def draw(combat_duration):
    ...
    for avatar in avatar_obj_dict.values():
        if lang == "chs":
            ...
            painter.text((x_px, y_px), "%s lv:%d" % (avatar.avatar_name, avatar.level), font=fnt, fill=avatar_info_color)
        y_px += 24
        if lang == "chs":
            ...
            painter.text((x_px, y_px),
                         "%s lv%d r%d" % (avatar.weapon.name, avatar.weapon.level, avatar.weapon.refinement)
                         , font=fnt, fill=avatar_info_color)
        y_px += 24
        painter.text((x_px, y_px),
    ...
    if lang == "chs":
        ...
        painter.text((x_px, y_px), "total:%.2fs    total DMG: %d" % (combat_duration / 1000, total_damage), font=fnt,
                     fill=avatar_info_color)
    x_px = 216
    y_px = 729
    if lang == "chs":
        ...
        painter.text((x_px, y_px), "crit in combat", font=fnt, fill=avatar_info_color)
        x_px += 142
        painter.text((x_px, y_px), "reaction ratio", font=fnt, fill=avatar_info_color)
        x_px += 142
        painter.text((x_px, y_px), "DMG ratio", font=fnt, fill=avatar_info_color)
    x_px = 86
    y_px = 800
    for avatar in avatar_obj_dict.values():
    ...
    if lang == "chs":
        ...
        plt.title("stamina sequence chart")
    ...
    if lang == "chs":
        ...
        plt.title("energy sequence chart")
    for avatar in avatar_obj_dict.values():
    ...
    if lang == "chs":
        ...
        copy_img.save("%s\\%s\\%s analysis %s.png" % (file_path, file_name_without_ext, file_name_without_ext, combat_round))


with open("./nahida_config.json", "r", encoding="utf-8") as f:
    config = json.load(f)
lang = config["language"]
avatar_id_dict = read_json("%s/avatar.json" % lang)
weapon_id_dict = read_json("%s/weapon.json" % lang)
reliquary_id_dict = read_json("%s/reliquary.json" % lang)
monster_id_dict = read_json("%s/monster.json" % lang)
fight_prop_dict = read_json("%s/fight_prop.json" % lang)
draw_fight_prop_dict = read_json("%s/draw_fight_prop.json" % lang)
element_id_dict = read_json("%s/element.json" % lang)
element_reaction_id_dict = read_json("%s/element_reaction.json" % lang)
root = Tk()
...
if lang == "chs":
    ...
    all_combat_text = open("%s\\%s\\%s analysis.txt" % (file_path, file_name_without_ext, file_name_without_ext), "w",
                           encoding="utf-8")
combat_round = 1
    ...
    if packet_name == "SceneTeamUpdateNotify":
        if combat_start and assume_end_time:
            ...
            if lang == "chs":
                ...
                all_combat_text = open(
                    "%s\\%s\\%s analysis_%s.txt" % (file_path, file_name_without_ext, file_name_without_ext, combat_round),
                    "w", encoding="utf-8")
            combat_start_time = 0
            attack_time_exactly = 0
            avatar_hit_times = {}
            ...
            if entity['entity_type'] == 'PROT_ENTITY_TYPE_MONSTER':
                ...
                if combat_start_time:
                    if lang == "chs":
                        ...
                        all_combat_text.write("%.2fs %s come on stage\n" % (
                        (ts - combat_start_time) / 1000, avatar_obj_dict[avatar_guid].avatar_name))
                else:
                    if lang == "chs":
                        ...
                        if str(attackee).startswith("33"):
                            if not combat_start:
                                ...
                                if lang == "chs":
                                    ...
                                    all_combat_text.write("combat start\n")
                            ...
                            if "damage" in attack_result:
                                ...
                                if "element_type" in attack_result:
                                    ...
                                    if lang == "chs":
                                        ...
                                        all_combat_text.write("%.2fs %s deal %d %s element hurt to %s,this attack %s, %s\n" % (
                                            (ts - combat_start_time) / 1000, attacker,
                                            int(damage), element_type, attackee_name,
                                            "crit" if crit else "didn't crit",
                                            "apply element" if apply_element else "didn't apply element"))
                                else:
                                    if lang == "chs":
                                        ...
                                        all_combat_text.write("%.2fs %s deal %d physical hurt to %s,this attack %s\n" % (
                                        (ts - combat_start_time) / 1000, attacker,
                                        int(damage), attackee_name, "crit" if crit else "didn't crit"))
                            else:
                                if lang == "chs":
                                    all_combat_text.write("%.2fs %s对%s造成无效伤害\n" % ((ts-combat_start_time)/1000, attacker, attackee_name))
                                elif lang == "en":
                                    all_combat_text.write("%.2fs %s's attack didn't deal damage to %s\n" % (
                                    (ts - combat_start_time) / 1000, attacker, attackee_name))
                    except KeyError:
                        pass
                    ...
                    if element_reaction_type in element_reaction_id_dict:
                        if element_reaction_type in [4, 5, 10, 11, 13, 14, 15, 26, 32]:
                            if lang == "chs":
                                ...
                                all_combat_text.write("%.2fs %s apply %s to %s\n" % (
                                    (ts - combat_start_time) / 1000, trigger_avatar,
                                    element_reaction_id_dict[element_reaction_type], entity_name))
                        elif element_reaction_type in [17, 18, 19, 20, 21, 22, 23, 24]:
                            if lang == "chs":
                                all_combat_text.write("%.2fs %s对%s触发了%s反应\n" % (
                                    (ts-combat_start_time) / 1000, trigger_avatar, entity_name, element_reaction_id_dict[element_reaction_type]))
                            elif lang == "en":
                                all_combat_text.write("%.2fs %s trigger %s reaction on %s\n" % (
                                    (ts - combat_start_time) / 1000, trigger_avatar,
                                    element_reaction_id_dict[element_reaction_type], entity_name))
                        else:
                            ...
                            if element_reactor_type in element_id_dict:
                                if element_source_type in element_id_dict:
                                    if lang == "chs":
                                        ...
                                        all_combat_text.write(
                                            "%.2fs %s trigger %s and %s %s reaction on %s\n" % ((ts - combat_start_time) / 1000,
                                                                                trigger_avatar,
                                                                                element_id_dict[element_source_type],
                                                                                element_id_dict[element_reactor_type],
                                                                                element_reaction_id_dict[
                                                                                    element_reaction_type], entity_name))
                                else:
        ...
        for prop_map in data['prop_map']:
            if 10011 in prop_map:
                ...
                if lang == "chs":
                    all_combat_text.write("%.2fs 当前体力为%.2f\n" % ((ts-combat_start_time)/1000 if combat_start_time else 0, prop_map[10011]["val"]/100))
                elif lang == "en":
                    all_combat_text.write(
                        "%.2fs stamina is %.2f now\n" % ((ts - combat_start_time) / 1000 if combat_start_time else 0, prop_map[10011]["val"] / 100))
            ...
            if invoke['argument_type'] == 'COMBAT_TYPE_ARGUMENT_BEING_HEALED_NTF':
                ...
                if 'real_heal_amount' in combat_data:
                    ...
                    if str(target_id).startswith("16"):
                        ...
                        if lang == "chs":
                            all_combat_text.write("%.2fs %s为%s治疗了%d血量\n" % ((ts-combat_start_time)/1000 if combat_start_time else 0, heal_avatar, target_avatar, int(heal_amount)))
                        elif lang == "en":
                            all_combat_text.write("%.2fs %s heal for %s %d hp\n" % (
                            (ts - combat_start_time) / 1000 if combat_start_time else 0, heal_avatar, target_avatar,
                            int(heal_amount)))
