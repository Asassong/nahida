...
def parse(decrypt_key):
    ...
    if lang == "chs":
        ...
        f_decrypt_data = open(now_time + "_combat_data.txt", "w", encoding="utf-8")
        print("Nahida work fine, file will save in %s_combat_data.txt, please use the Akasha to analysis it." % now_time)
    while True:
        if i <= len(packet) - 1:
            get = False
            try:
...
if lang == "chs":
    ...
    now_time = time.strftime("%B_%d_%H_%M")
